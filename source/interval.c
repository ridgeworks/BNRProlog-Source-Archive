/*
*
*  $Header: /disc9/sniff/repository/BNRProlog/source/RCS/interval.c,v 1.4 1998/01/20 15:05:14 thawker Exp $
*
*  $Log: interval.c,v $
 * Revision 1.4  1998/01/20  15:05:14  thawker
 * Added previously missing roundnearest definition for solaris
 *
 * Revision 1.3  1996/02/09  00:51:11  yanzhou
 * A bug fixed in nextAddr().
 *
 * Revision 1.2  1995/12/04  00:10:17  yanzhou
 * Now supports IBM Power/RS6000 AIX 4.1.
 *
 * Revision 1.1  1995/09/22  11:24:50  harrisja
 * Initial version.
 *
*
*/
...
#include <float.h>		/* for DBL_MAX and fp_swap_rnd() */
#define int_fp          fp
#define resetfpu()		fp_swap_rnd(FP_RND_RN)
#define initfpu()		resetfpu(); maxfp = DBL_MAX
#define iszero(x)       ((x EQ -0.0) || (x EQ 0.0))
#define roundnearest(x)	resetfpu(); x = rint(x)
#define next(x)         __ieeenext(x)
#define prev(x)         __ieeeprev(x)
#define upperbd(y, x)   fp_swap_rnd(FP_RND_RP); x = y
#define lowerbd(y, x)   fp_swap_rnd(FP_RND_RM); x = y
#define upperbd2(y, x)  resetfpu(); x = y; next(x)
#define lowerbd2(y, x)  resetfpu(); x = y; prev(x)
#endif  /* _AIX */
...
#define nextAddr(p, x, a)	while (tagof(x = *p) EQ TVTAG) { \
                ...
                while (isVAR(x)) { \
                    long _tt; \
                    a = addrof(x); /* save address part before deref */ \
                    _tt = derefVAR(x); \
                    if (x EQ _tt) break; \
                    x = _tt; \
                }
