	...
	*	================================		*
	*							*
	*	Program:	Test Receiver			*
	*	Version:	2.1.0				*
	*	File:		'rtest_rec.p'			*
	*	Written:	I Sykes		Date: 12/1/98	*
	*	Revised:			Date:		*
	*							*
	\*******************************************************/
...
%	PROCESS TERMS...
		...
		receive_term(SktRcv,SktSnd),

%	& CLOSE UP SHOP.
		closesocket(SktRcv),
		closesocket(SktSnd),
	write("\n\nReceiver terminated!\n\n").
test_rec:- write("\nTest Receiver main clause 'test_rec' has failed.\n").
...
%	EXECUTE TERMS, SIGNALS TERM FAILURE OR TERMINATES PROGRAM AS REQUIRED.	
		...
		execute(X,go):- 
			write("Resolving term '",X,"'\n"),
			X,
			write("Term '",X,"' succeeded.\n"),!.
		execute(X,go):- write("Term '",X,"' failed!\n").
