/*
*
*  $Header: /disc9/sniff/repository/BNRProlog/build_base/RCS/base_listener.p,v 1.4 1998/06/05 05:59:49 csavage Exp $
*
*  $Log: base_listener.p,v $
 * Revision 1.4  1998/06/05  05:59:49  csavage
 * Changed file name of BNR Prolog Console to
 * remove spaces.. BNRPrologConsole
 *
 * Revision 1.3  1996/08/28  10:56:21  harrisj
 * $console modified to support renaming of console window.
 *
 * Revision 1.2  1996/07/04  13:23:05  yanzhou
 * Removed the word "Alpha" from the version message.
 *
 * Revision 1.1  1995/09/22  11:21:33  harrisja
 * Initial version.
 *
*
*/
...
%%% Default Listener
%%%
%%% This listener assumes simple console operation as provided by line buffered
%%% Unix/Lightspeed defaults. It doesn't work very well with more intelligent
%%% screen based interfaces, e.g., MPW.  It also works with the event driven
%%% interface.
%%%
...
$listener_read(Pipe, Term, Echo, Op) :-
    new_counter(0, C),
	$listen_convience_prompt,			% display the prompt
	repeat,
	C,
...
$console(Console) :-
    recall(bnrpConsoleWindow(Console), $local),!.
$console("BNRPrologConsole") :-
    remember(bnrpConsoleWindow("BNRPrologConsole"), $local).
    
$newConsoleName(NewName) :-
    update(bnrpConsoleWindow(_), bnrpConsoleWindow(NewName), $local).
