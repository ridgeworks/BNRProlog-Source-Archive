/*
*
*  $Header: /disc9/sniff/repository/BNRProlog/utilities/RCS/debugger_CAF.p,v 1.2 1997/03/20 17:09:49 harrisj Exp $
*
*  $Log: debugger_CAF.p,v $
 * Revision 1.2  1997/03/20  17:09:49  harrisj
 ...
 *
 * Revision 1.1  1995/09/22  11:23:49  harrisja
 * Initial version.
 *
*
*/
...
%%%RWW  slee based listener  $findgoal(Level, listener), !,                         % find the current listener
    $findlevel(Level), !,                          % find the current listener
...
/*** RWW replaced for SLEE based listeners who don't have "listener" as ancestor
$findgoal(Addr, Goal) :- 
    goal(0, _, FirstAddr),
...
% RWW replacement follows.. */

$findlevel(Addr) :- 
    goal(0, _, FirstAddr),
    $recurse(FirstAddr, Addr),!.
$findlevel(-1).  % couldn't find address, maybe injected message,return -1
...
$recurse(FirstAddr, Addr) :-
    goal(FirstAddr, _, NewAddr),
    $recurse(NewAddr, Addr).

$listenergoal(listener).     % standard PDE
$listenergoal(listener_msg). % CAF driven 
%%% RWW end of replacement
